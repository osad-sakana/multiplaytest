...
from models import GameState, GameUpdate, Player, PlayerInput
...
class GameManager:
    ...
    async def add_player(self, websocket, player_name: str) -> Player:
        ...
        player = Player(name=player_name, x=spawn_x, y=spawn_y)

        self.state.players[player.id] = player
        self.connected_clients[player.id] = websocket

        # Notify all clients about new player
        update = GameUpdate(type="player_joined", data={"player": player.dict()})
    ...
    async def move_player(self, player: Player, direction: str):
        ...
        if direction == "up":
            ...
            player.y = min(
                self.state.field_height - self.state.player_size, player.y + speed
            )
        elif direction == "left":
            player.x = max(0, player.x - speed)
        elif direction == "right":
            player.x = min(
                self.state.field_width - self.state.player_size, player.x + speed
            )
    ...
    async def dash_player(self, player: Player, direction: str, current_time: float):
        ...
        if direction == "up":
            player.y = max(
                -50, player.y - dash_distance
            )  # Allow going slightly off-screen
        elif direction == "down":
    ...
    def is_out_of_bounds(self, player: Player) -> bool:
        return (
            player.x < -self.state.player_size
            or player.x > self.state.field_width
            or player.y < -self.state.player_size
            or player.y > self.state.field_height
        )
    ...
    async def respawn_player(self, player: Player):
        ...
        update = GameUpdate(type="respawn", data={"player": player.dict()})
        await self.broadcast_update(update)

    async def broadcast_player_update(self, player: Player):
        update = GameUpdate(type="player_update", data={"player": player.dict()})
        await self.broadcast_update(update)
